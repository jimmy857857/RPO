...
App({
  ...
  checkPrivacyPolicy() {
    ...
    const privacyAgreeTime = wx.getStorageSync('privacyAgreeTime');
    
    // 如果从未同意过隐私政策，或者同意时间超过30天需要重新确认
    if (!privacyAgreed || !privacyAgreeTime || 
        (Date.now() - new Date(privacyAgreeTime).getTime()) > 30 * 24 * 60 * 60 * 1000) {
    ...
    if (wx.getUserProfile) {
      wx.getUserProfile({
        ...
        success: (res) => {
          ...
          // 记录用户同意时间
          wx.setStorageSync('userInfoAgreeTime', new Date())
      ...
      wx.showModal({
        title: '提示',
        content: '当前微信版本过低，无法使用该功能，请升级到最新微信版本。',
        showCancel: false
      })
      if (callback) callback(null)
    }
